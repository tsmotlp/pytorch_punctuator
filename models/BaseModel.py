...
import torch
import os
from utils import set_seed, tensor2image
...
class BaseModel(ABC):
    ...
    def get_current_visual_images(self):
        """Return visualization images. trainer.py will display these images with visdom, and save the images to a HTML"""
    ...
    def get_current_visual_losses(self):
        """Return traning losses / errors. trainer.py will print out these errors on console, and save them to a file"""
    ...
    def save_images(self, images):
        for name, image in images.items():
            ...
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            save_filename = '%s%s_epoch%s.png' % (name, self.image_name[0].split(".")[0], self.args.load_epoch)    # batch_size=1
            save_path = os.path.join(save_dir, save_filename)
