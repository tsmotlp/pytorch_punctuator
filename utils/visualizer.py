...
class Logger():
    ...
    def log(self, losses=None, images=None, epoch=1):
        ...
        sys.stdout.write(
            '\rEpoch [%03d/%03d] Batch [%05d/%05d] -- ' % (epoch, self.n_epochs, self.batch, self.batches_epoch))
        ...
        for image_name, tensor in images.items():
            if image_name not in self.image_windows:
                self.image_windows[image_name] = self.viz.image(tensor.data, opts={'title': image_name})
            else:
                self.viz.image(tensor.data, win=self.image_windows[image_name],
